...
## 1. Why Virtual Memory Exists (the problems it solves)
...
> Translation happens on **every single memory access** — so it must be fast (see TLB, §6).
...
*Previous topic: **Process vs Thread at the OS level**.*
*Next topic in roadmap: **Context switching**.*
