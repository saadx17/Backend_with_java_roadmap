...
# `SELECT` - QUERY CHEAT SHEET
```
1. SELECT          → Choose columns
2. DISTINCT        → Unique rows only
3. AS              → Rename columns/tables
4. FROM            → Source table
5. WHERE           → Filter rows
   AND / OR / NOT  → Combine conditions
   IN / NOT IN     → Match list
   BETWEEN         → Range (inclusive)
   LIKE (% _)      → Simple pattern
   REGEXP          → Complex pattern
   IS NULL         → Check for null
6. ORDER BY        → Sort (ASC/DESC)
7. LIMIT / OFFSET  → Pagination

Conditional:
  CASE WHEN...THEN...ELSE...END  → Multi-branch logic
  IF(cond, true, false)          → Simple ternary
  IFNULL(val, default)           → NULL → default
  NULLIF(a, b)                   → Equal → NULL
...
ORDER OF EXECUTION (not writing order):
