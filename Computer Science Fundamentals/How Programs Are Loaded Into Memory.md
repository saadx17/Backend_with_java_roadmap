...
Before anything can be loaded, source code becomes an executable file.
The build pipeline:
```
  source.c
...
**Static vs dynamic linking** (affects loading):
...
|  .bss    (uninit. globals) |  zero-initialized vars
|                            |      (takes NO file space)  
+----------------------------+
...
1. What is the difference between a _program_ and a _process_?
