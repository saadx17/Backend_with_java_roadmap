...
## 2. The ALU
The **Arithmetic Logic Unit** is the CPU's **calculator**, it performs the actual computations:
...
Most computers follow the **Von Neumann architecture**: instructions and data share the **same memory** and travel over the **same bus** to the CPU (invented 1945, still used in every modern computer)
```
Memory is just a giant array of bytes:
Address: 0x0000  0x0001  0x0002  0x0003  0x0004  0x0005 ...
Content: [0x48]  [0x89]  [0xC8]  [0x01]  [0xF0]  [0x90] ...
          ↑                              ↑
    an instruction                   some data
    (MOV RAX, RCX)                (integer value)
```
