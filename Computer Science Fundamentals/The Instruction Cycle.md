The **instruction cycle** (also known as the fetch-decode-execute cycle) is the fundamental operational process of a CPU.
From the moment a computer boots up until it shuts down, the CPU continuously loops through these core phases to process instructions:
1. [[#3. FETCH|Fetch]] Cycle
2. [[#4. DECODE|Decode]] Cycle
3. [[#5. EXECUTE|Execute]] Cycle
4. [[#6. STORE / WRITE-BACK|Store/Write Back]] Cycle
...
```
Memory: a=5 at adrs 100, b=3 at adrs 104, c at adrs 108
```
...
Source: multiply(6,7)
    ...
    ▼
Compiler (javac):
  javac translates to bytecode
  BYTECODE:
    iload_1     (push a onto stack)
    iload_2     (push b onto stack)
    imul        (multiply top 2 stack values)
    ireturn     (return int)
    │
    ▼
JVM Startup:
  Interprets bytecode (slow but immediate)
  Counts invocations
    │
    ▼
JIT Compilation (after ~10,000 calls):
  C1 compiler: quick compile, basic optimizations
  C2 compiler (after more calls): full optimization
    - Inlining (copy method body into caller)
    - Register allocation (put a,b in CPU registers)
    - Convert imul bytecode → IMUL machine instruction
    - Eliminate unnecessary MOVs
    │
    ▼
Machine Code stored in Code Cache:
  48 0F AF FE   IMUL RDI, RSI
  89 F8         MOV  EAX, EDI
  C3            RET
    │
    ▼
CPU FETCH:
  PC → address of IMUL
  L1i cache → instruction bytes loaded
  PC incremented
    │
    ▼
CPU DECODE:
  Bytes → μops
  IMUL μop → Reservation Station
    │
    ▼
CPU EXECUTE:
  Out-of-order engine schedules μop
  Multiplication unit: 6 × 7 = 42
  3 cycles
    │
    ▼
WRITE BACK:
  EDI = 42
  ROB retires instruction
  EAX = 42 (return value)
    │
    ▼
RESULT: 42 returned to Java caller
```

## 11. How This Maps to Java / the JVM
